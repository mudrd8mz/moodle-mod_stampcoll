...
require_once(dirname(__FILE__).'/lib.php');
    ...
    function stampcoll_backup_one_mod($bf, $preferences, $stampcoll) {
        ...
        $status = $status && fwrite($bf, full_tag('PUBLISH', 4, false, $stampcoll->publish));
        $status = $status && fwrite($bf, full_tag('TIMEMODIFIED', 4, false, $stampcoll->timemodified));
        $status = $status && fwrite($bf, full_tag('TEACHERCANCOLLECT', 4, false, $stampcoll->teachercancollect));
    ...
    function stampcoll_backup_collected_stamps($bf, $preferences, $stampcollid) {
        ...
        foreach ($stamps as $stamp) {
            ...
            $status = $status && fwrite($bf, full_tag('COMMENT', 6, false, $stamp->comment));
